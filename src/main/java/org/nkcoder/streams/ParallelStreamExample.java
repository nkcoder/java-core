...
public class ParallelStreamExample {
  ...
  static void basicParallelStream() {
    ...
    // Check if parallel
    System.out.println(
        "Is parallel, parallel1: "
            + parallel1.isParallel()
            + ", parallel2: "
            + parallel2.isParallel());
    ...
    numbers.parallelStream()
        .forEach(n -> System.out.println("  " + n + " on " + Thread.currentThread().getName()));
  ...
  static void parallelVsSequential() {
    ...
    long sumSeq = IntStream.range(0, size).mapToLong(i -> (long) Math.sqrt(i)).sum();
    long timeSeq = System.currentTimeMillis() - startSeq;

    // Parallel
    long startPar = System.currentTimeMillis();
    long sumPar = IntStream.range(0, size).parallel().mapToLong(i -> (long) Math.sqrt(i)).sum();
  ...
  /**
   * Parallel streams track the source's encounter order and enforce it when the terminal operation
   * requires ordering.
   *
   * <p>Encounter order (a.k.a. stream order) is the well-defined sequence in which a stream's
   * source presents elements. An ordered stream preserves that sequence through the pipeline unless
   * you make it unordered.
   *
   * <pre>
  ...
  static void whenToUseParallel() {
    ...
    System.out.println(
        "Small dataset - Sequential: " + seqTime + "ns, Parallel: " + parTime + "ns");
  }
