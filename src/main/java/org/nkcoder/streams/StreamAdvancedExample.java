 ...
 *
 * <ul>
 *   <li>{@code flatMap}: flatten nested structures
 *   <li>{@code reduce}: combine elements into single result
 *   <li>{@code takeWhile/dropWhile}: conditional slicing (Java 9+)
 *   <li>{@code teeing}: collect into two collectors at once (Java 12+)
 * </ul>
 */
public class StreamAdvancedExample {
    ...
    static void flatMapExample() {
        ...
        List<String> words =
                sentences.stream().flatMap(s -> Stream.of(s.split(" "))).toList();
    ...
    static void takeWhileDropWhile() {
        ...
        // Combine for "window"
        List<Integer> window =
                numbers.stream().dropWhile(n -> n < 3).takeWhile(n -> n <= 5).toList();
    ...
    static void teeingCollector() {
        ...
        Partition partition = numbers.stream()
                .collect(Collectors.teeing(
    ...
    static void gatherersPreview() {
        ...
        List<List<Integer>> windows = IntStream.range(0, (numbers.size() + windowSize - 1) / windowSize)
                .mapToObj(i -> numbers.subList(i * windowSize, Math.min((i + 1) * windowSize, numbers.size())))
                .toList();
