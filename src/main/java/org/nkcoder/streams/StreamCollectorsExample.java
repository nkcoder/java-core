...
public class StreamCollectorsExample {
    ...
    static void basicCollectors() {
        ...
        List<String> names = people().stream().map(Person::name).toList(); // .collect(Collectors.toList())
        ...
        // Counting
        long count = people().stream().filter(p -> p.age > 10).count(); // .collect(Collectors.counting())
        System.out.println("Count: " + count);

        // Summing
        int totalAge = people().stream().mapToInt(Person::age).sum(); // .collect(Collectors.summingInt(Person::age))
    ...
    static void toMapCollector() {
        ...
        Map<String, Integer> cityToCountSorted =
                people().stream().collect(Collectors.toMap(Person::city, p -> 1, Integer::sum, TreeMap::new));
    ...
    static void groupByCollector() {
        ...
        Map<String, Map<String, List<Person>>> byCityThenAge = people().stream()
    ...
    static void partitioningByCollector() {
        ...
        Map<Boolean, List<Person>> byAge30 = people().stream().collect(Collectors.partitioningBy(p -> p.age < 30));
        System.out.println("Age >= 30: " + byAge30.get(true));
        System.out.println("Age < 30: " + byAge30.get(false));

        // With downstream collector
        Map<Boolean, Long> countByAge30 =
                people().stream().collect(Collectors.partitioningBy(p -> p.age >= 30, Collectors.counting()));
    ...
    static void joiningCollector() {
        ...
        String formatted = people().stream().map(Person::name).collect(Collectors.joining(", ", "[", "]"));
    ...
    static void downstreamCollector() {
        ...
        Map<String, Integer> sumAgeByCity =
                people().stream().collect(Collectors.groupingBy(Person::city, Collectors.summingInt(Person::age)));
