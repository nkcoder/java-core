 ...
 *
 * <p><strong>Java 25 Status:</strong> Core language feature, stable. Exception chaining is essential for debugging and
 * understanding failure root causes.
 *
 * <p>Key concepts:
 *
 * <ul>
 *   <li>Cause: The exception that triggered this exception
 *   <li>Suppressed: Exceptions that occurred during cleanup
 *   <li>Stack trace preservation
 *   <li>Best practices for exception wrapping
 * </ul>
 */
public class ExceptionChainingExample {
    ...
    static void multiLevelChaining() {
        ...
        try {
            ...
            while (current != null) {
                System.out.println(
                        "    " + "  ".repeat(level) + current.getClass().getSimpleName() + ": " + current.getMessage());
    ...
    static void analyzingStackTraces() {
        ...
        try {
            ...
            for (int i = 0; i < Math.min(3, elements.length); i++) {
                ...
                System.out.println(
                        "    " + el.getClassName() + "." + el.getMethodName() + " (line " + el.getLineNumber() + ")");
            }
        }
    ...
    static void wrappingPatterns() {
        ...
        try {
            throw new RetryableException("Service unavailable", new IOException("Timeout"), true, 5000);
        } catch (RetryableException e) {
            System.out.println("  Retryable: " + e.getMessage());
            System.out.println("    Can retry: " + e.isRetryable() + ", delay: " + e.getSuggestedDelayMs() + "ms");
        }
