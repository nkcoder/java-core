 ...
 *
 * <p><strong>Java 25 Status:</strong> All methods shown are stable and recommended. Files.walk() and Files.list() with
 * Streams are the modern approach.
 *
 * <p>Key concepts:
 *
 * <ul>
...
public class DirectoryOperationsExample {
    ...
    static void listDirectory() throws IOException {
        ...
        if (Files.exists(srcDir)) {
            ...
            try (Stream<Path> stream = Files.list(srcDir)) {
                stream.sorted().limit(10).forEach(p -> {
    ...
    static void walkDirectory() throws IOException {
        ...
        if (Files.exists(srcDir)) {
            ...
            try (Stream<Path> stream = Files.walk(srcDir)) {
                var counts = stream.filter(Files::isRegularFile)
                        .map(p -> {
    ...
    static void findFiles() throws IOException {
        ...
        if (Files.exists(srcDir)) {
            ...
            try (Stream<Path> stream = Files.find(
                    ...
                    Integer.MAX_VALUE,
                    (path, attrs) ->
                            attrs.isRegularFile() && path.toString().endsWith(".java") && attrs.size() > 5000)) {
        ...
        if (Files.exists(srcDir)) {
            ...
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(
                    ...
                    entry -> Files.isDirectory(entry)
                            && entry.getFileName().toString().startsWith("s"))) {
                for (Path path : stream) {
                    System.out.println("    " + path.getFileName());
                }
            }
