 ...
 *
 * <p><strong>Java 25 Status:</strong> The HttpClient API is stable and recommended. It replaces the legacy
 * HttpURLConnection for all new code.
 *
 * <p>Key concepts:
 *
 * <ul>
...
public class HttpClientExample {
    ...
    static void requestConfiguration() {
        ...
        request.headers().map().forEach((name, values) -> System.out.println("      " + name + ": " + values));
    ...
    static void responseHandling() throws IOException, InterruptedException {
        ...
        HttpResponse<Stream<String>> linesResponse = client.send(request, HttpResponse.BodyHandlers.ofLines());
        ...
        System.out.println("    discarding(): body discarded, status = " + discardResponse.statusCode());

        // Response headers
        System.out.println("\n  Accessing headers:");
        var headers = stringResponse.headers();
        System.out.println(
                "    Content-Type: " + headers.firstValue("content-type").orElse("N/A"));
    ...
    static void asyncRequests() throws ExecutionException, InterruptedException {
        ...
        System.out.println("\n  Chaining operations:");
        HttpRequest quickRequest =
                HttpRequest.newBuilder().uri(URI.create(TEST_URL + "/get")).build();

        String result = client.sendAsync(quickRequest, HttpResponse.BodyHandlers.ofString())
