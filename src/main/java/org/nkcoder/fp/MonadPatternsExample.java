 ...
 *
 * <ul>
 *   <li>Try monad: Wrap exceptions in functional style
 *   <li>Validation: Accumulate multiple errors (vs fail-fast)
 *   <li>Railway-oriented programming: Chain operations that may fail
 *   <li>Real-world pipeline: Parse → Validate → Transform → Save
 * </ul>
 *
...
public class MonadPatternsExample {
    ...
    static void tryMonad() {
        ...
        Try<Integer> result =
                Try.of(() -> Integer.parseInt("10")).map(n -> n * 2).map(n -> n + 5);
        ...
        Try<Integer> division = Try.of(() -> Integer.parseInt("100")).flatMap(n -> divide(n, 5));
        System.out.println("100 / 5 = " + division);

        Try<Integer> divByZero = Try.of(() -> Integer.parseInt("100")).flatMap(n -> divide(n, 0));
    ...
    static Try<Integer> divide(int a, int b) {
        return b == 0 ? Try.failure(new ArithmeticException("Division by zero")) : Try.success(a / b);
    }
    ...
    record Valid<E, T>(T value) implements Validation<E, T> {
        ...
        @Override
        public <U, V> Validation<E, V> combine(
                Validation<E, U> other, java.util.function.BiFunction<T, U, V> combiner) {
            if (other.isValid()) {
    ...
    static void railwayOriented() {
        ...
        Result<Integer, String> success =
                parseNumber("42").flatMap(n -> validatePositive(n)).map(n -> n * 2);
        System.out.println("Parse '42', validate positive, double: " + success);

        System.out.println("\n-- Failure at Parse --");
        Result<Integer, String> failParse =
                parseNumber("bad").flatMap(n -> validatePositive(n)).map(n -> n * 2);
        System.out.println("Parse 'bad': " + failParse);

        System.out.println("\n-- Failure at Validate --");
        Result<Integer, String> failValidate =
                parseNumber("-5").flatMap(n -> validatePositive(n)).map(n -> n * 2);
        System.out.println("Parse '-5', validate positive: " + failValidate);

        // Transform errors
        System.out.println("\n-- Error Transformation --");
        Result<Integer, Error> typedError = parseNumber("bad").mapError(msg -> new Error("PARSE_ERROR", msg));
    ...
    static Result<Integer, String> validatePositive(int n) {
        return n > 0 ? Result.ok(n) : Result.err("Number must be positive, got: " + n);
    }
    ...
    static Result<String, String> saveOrder(ProcessedOrder order) {
        ...
        return Result.ok(
                "Order " + order.id + " saved successfully (total: $" + String.format("%.2f", order.total) + ")");
    }
