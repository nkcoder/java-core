 ...
 *
 * <p>Concept: CPU caches are organized in lines (usually 64 bytes). When a core reads a memory location, it fetches the
 * entire cache line. If two independent variables (e.g., fields of an object) sit on the same cache line and are
 * updated by different cores, the cores will fight for ownership of that cache line (Cache Coherency Protocols like
 * MESI). This causes severe performance degradation, known as "False Sharing".
 *
 * <p>This example compares: 1. Padded structure (preventing false sharing by spacing out fields) 2. Unpadded structure
 * (suffering from false sharing)
 *
 * <p>To run: {@code java org.nkcoder.jvm.FalseSharingExample}
 */
