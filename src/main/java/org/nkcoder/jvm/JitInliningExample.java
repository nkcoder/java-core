 ...
 *
 * <p>
 * Concept: The JVM starts by interpreting bytecode. Hot methods (executed frequently) are compiled
 * to native code by the C1 and C2 compilers.
 *
 * <p>
 * Optimizations shown: 1. **Inlining**: Replacing a method call with the method body itself to save
 * overhead. 2. **Dead Code Elimination**: Removing code that has no side effects or whose result is
 * unused.
 *
 * <p>
 * To observe JIT in action, run with VM flags:
 * {@code -XX:+PrintCompilation -XX:+UnlockDiagnosticVMOptions -XX:+PrintInlining} (Note: precise
 * flags depend on Java version)
 */
