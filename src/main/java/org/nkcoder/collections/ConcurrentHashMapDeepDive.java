 ...
 *
 * <p>
 * Key Concepts: 1. **Thread Safety != Atomic Logic**: Just because the map is thread-safe doesn't
 * mean your logic is. Using `get` then `put` exhibits race conditions (Check-Then-Act). 2.
 * **Correct Atomic Operations**: {@code computeIfAbsent}, {@code compute}, {@code merge}. 3.
 * **Internals**: Segment Locking (Java 7) vs CAS + Node Locking (Java 8+).
 */
public class ConcurrentHashMapDeepDive {
  ...
  static void testRaceCondition() throws InterruptedException {
    ...
    System.out.println(
        "Result:   " + (map.get("key").equals(expectedTotal) ? "PASS" : "FAIL (Race Condition)"));
  }
  ...
  static void testCorrectUsage() throws InterruptedException {
    ...
    System.out
        .println("- If collision occurs, it locks ONLY that specific bucket/node (synchronized).");
    System.out.println(
        "- This allows extremely high concurrency compared to Collections.synchronizedMap().");
  }
}
