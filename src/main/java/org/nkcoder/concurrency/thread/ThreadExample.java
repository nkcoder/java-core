 ...
 *
 * <p><strong>Java 25 Note:</strong> Raw threads are foundational knowledge but rarely used directly
 * in modern code. Prefer {@code Executors.newVirtualThreadPerTaskExecutor()} for I/O-bound tasks
 * or {@code ExecutorService} for thread pool management.
 *
 * <p>Key concepts:
 * <ul>
 *   <li>Two ways to create threads: extend Thread or implement Runnable</li>
 *   <li>Thread lifecycle: NEW -> RUNNABLE -> RUNNING -> BLOCKED/WAITING -> TERMINATED</li>
 *   <li>Thread priority is a hint, not guaranteed</li>
 *   <li>join() waits for thread completion</li>
 * </ul>
 *
 * <p>Interview tip: Know the difference between Runnable and Thread,
 * and understand thread states.
 */
public class ThreadExample {
  ...
  static void creatingThreads() throws Exception {
    ...
    Thread t4 = Thread.ofPlatform()
        .name("custom-thread")
        .start(() -> {
          System.out.println("  [4] Platform thread builder: " + Thread.currentThread().getName());
        });
    t4.join();
