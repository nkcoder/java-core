 ...
 *
 * <p><strong>Java 25 Note:</strong> Still useful for read-heavy workloads where reads significantly outnumber writes.
 * For simpler cases, consider {@code ConcurrentHashMap} or {@code Atomic*}. {@code StampedLock} offers optimistic reads
 * for even better read performance.
 *
 * <p>Key concepts:
 *
 * <ul>
 *   <li>Multiple threads can hold read lock simultaneously
 *   <li>Write lock is exclusive (no other readers or writers)
 *   <li>Great for read-heavy workloads with occasional writes
 *   <li>StampedLock (Java 8+) offers optimistic reads
 * </ul>
 *
 * <p>Interview tip: Know when ReadWriteLock outperforms synchronized, and understand the writer starvation problem.
 */
