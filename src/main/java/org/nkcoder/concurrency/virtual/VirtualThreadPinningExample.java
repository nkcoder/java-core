 ...
 *
 * <p>Concept: Virtual threads are mounted on "carrier threads" (Platform threads). When a virtual thread blocks (e.g.,
 * Thread.sleep, I/O), it should "unmount" so the carrier can run other virtual threads.
 *
 * <p>The Problem (Pinning): If a virtual thread blocks inside a `synchronized` block (or native method), it cannot
 * unmount. The carrier thread remains blocked ("pinned"). This limits throughput to the number of carrier threads (OS
 * threads), defeating the purpose of virtual threads.
 *
 * <p>The Solution: Use {@link java.util.concurrent.locks.ReentrantLock} instead of `synchronized` for guarding critical
 * sections involving blocking operations.
 *
 * <p>Note: JEP 425 and subsequent updates are working to reduce pinning scenarios (e.g., in Object.wait), but
 * `synchronized` usage is the classic offender.
 */
public class VirtualThreadPinningExample {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("NOTE: For clear demonstration, run with: -Djdk.virtualThreadScheduler.parallelism=1");
