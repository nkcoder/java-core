 ...
 *
 * <p><strong>Java 25 Note:</strong> Still the right choice for limiting concurrent access to resources (connection
 * pools, rate limiting). No direct modern replacement exists.
 *
 * <p>Key concepts:
 *
 * <ul>
 *   <li>Maintains a set of permits
 *   <li>acquire() takes a permit (blocks if none available)
 *   <li>release() returns a permit
 *   <li>Unlike locks, permits can be released by different thread
 * </ul>
 *
 * <p>Interview tip: Know the difference between Semaphore and Lock. Classic use case: connection pool, rate limiting.
 */
