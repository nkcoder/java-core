 ...
 *
 * <p><strong>Java 25 Status:</strong> All methods shown are stable and widely used. These are the go-to String methods
 * for everyday Java development.
 *
 * <p>Key concepts:
 *
 * <ul>
 *   <li>Blank checking and whitespace handling
 *   <li>Lines and stream processing
 *   <li>Indentation and transformation
 *   <li>Repetition and stripping
 * </ul>
 */
public class StringMethodsExample {
    ...
    static void linesMethods() {
        ...
        multiline.lines().map(line -> "    > " + line).forEach(System.out::println);

        // Practical: Count non-blank lines
        String withBlanks = "Line 1\n\nLine 2\n   \nLine 3";
        long nonBlankCount = withBlanks.lines().filter(line -> !line.isBlank()).count();
    ...
    static void indentAndTransform() {
        ...
        String processed = json.transform(s -> s.isBlank() ? "{}" : s).transform(String::strip);
    ...
    static void otherUsefulMethods() {
        ...
        String collected = java.util.List.of("A", "B", "C").stream().collect(Collectors.joining(" | ", "[", "]"));
