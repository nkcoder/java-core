 ...
 *
 * <p><strong>Java 25 Status:</strong> Finalized in Java 21 and production-ready. Enables
 * powerful destructuring of record components in instanceof and switch.
 *
 * <p>Key concepts:
 * <ul>
 *   <li>Deconstruct record components in one step</li>
 *   <li>Nested patterns for deep matching</li>
 *   <li>Works with instanceof and switch</li>
 *   <li>var can infer component types</li>
 * </ul>
 */
public class RecordPatternExample {
  ...
  static void nestedPatterns() {
    ...
    String result = switch (shape) {
      case Triangle(
        ...
        yield "Triangle with vertices (" + ax + "," + ay + "), " +
            "(" + bx + "," + by + "), (" + cx + "," + cy + ")";
      }
  ...
  static void withInstanceof() {
    ...
    for (Object shape : shapes) {
      if (shape instanceof Point(int x, int y)) {
        ...
        System.out.println("  Rectangle: origin=(" + x + "," + y + "), " + w + "x" + h +
            ", area=" + (w * h));
      } else {
  ...
  static void withSwitch() {
    ...
    Shape[] shapes = {
        new PointShape(5, 10),
        new CircleShape(0, 0, 5),
        new RectangleShape(10, 20, 30, 40)
    };
  ...
  static void realWorldExamples() {
    ...
    // (x + 5) * 2
    Expr expr = new BinOp("*",
        new BinOp("+", new Var("x"), new Const(5)),
        new Const(2)
    );
    ...
    String response = switch (req) {
      case HttpRequest(String m, String p, var params)
          when m.equals("GET") && p.startsWith("/users/") ->
          "Fetching user: " + p.substring(7);
      case HttpRequest(String m, String p, var params)
          when m.equals("POST") && p.equals("/users") ->
          "Creating user";
      case HttpRequest(var m, var p, var params) ->
          "Unknown route: " + m + " " + p;
    };
