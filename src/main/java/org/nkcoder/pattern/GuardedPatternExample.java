 ...
 *
 * <p><strong>Java 25 Status:</strong> Finalized in Java 21 and production-ready. The 'when'
 * keyword allows adding boolean conditions to pattern cases for more precise matching.
 *
 * <p>Key concepts:
 * <ul>
 *   <li>when clause adds conditions to pattern cases</li>
 *   <li>Pattern variable is available in the when condition</li>
 *   <li>Guarded patterns must come before unguarded ones</li>
 *   <li>Enables complex matching logic within switch</li>
 * </ul>
 */
public class GuardedPatternExample {
  ...
  static void basicWhenClause() {
    ...
    for (Object input : List.of("", "Hi", "Hello, World!", "A".repeat(50), 42)) {
      ...
      System.out.println("    " + (input == null ? "null" : input.toString().substring(0, Math.min(10, input.toString().length()))) + " -> " + desc);
    }
  ...
  static void withRecordPatterns() {
    ...
    for (Circle c : circles) {
      String desc = switch (c) {
        ...
        case Circle(Point(int x, int y), int r)
            when Math.sqrt(x * x + y * y) > 50 ->
            "Circle far from origin at (" + x + "," + y + ")";
  ...
  static void realWorldExamples() {
    ...
    for (HttpResponse r : responses) {
      String action = switch (r) {
        case HttpResponse(int s, String b) when s >= 200 && s < 300 && !b.isEmpty() ->
            "Success with body";
        case HttpResponse(int s, String b) when s >= 200 && s < 300 ->
            "Success (no body)";
        case HttpResponse(int s, String b) when s == 400 ->
            "Bad request: " + b;
        case HttpResponse(int s, String b) when s == 404 ->
            "Not found";
        case HttpResponse(int s, String b) when s >= 500 ->
            "Server error: " + b;
    ...
    System.out.println("\n  Event scheduling:");
    for (Event e : events) {
      String timing = switch (e) {
        case Event(String name, LocalDate d) when d.isBefore(today) ->
            name + " - Already passed";
        case Event(String name, LocalDate d) when d.isEqual(today) ->
            name + " - Happening today!";
        case Event(String name, LocalDate d) when d.isBefore(today.plusDays(7)) ->
            name + " - This week";
        case Event(String name, LocalDate d) when d.isBefore(today.plusMonths(1)) ->
            name + " - This month";
    ...
    for (UserInput input : inputs) {
      String validation = switch (input) {
        case UserInput(String f, String v) when f.equals("email") && v.isEmpty() ->
            "Email is required";
        case UserInput(String f, String v) when f.equals("email") && v.contains("@") ->
            "Email OK: " + v;
        case UserInput(String f, String v) when f.equals("email") ->
            "Invalid email format";

        case UserInput(String f, String v) when f.equals("age") && v.matches("\\d+") -> {
          int age = Integer.parseInt(v);
          yield age >= 0 && age < 150 ? "Age OK: " + age : "Invalid age value";
        }
        case UserInput(String f, String v) when f.equals("age") ->
